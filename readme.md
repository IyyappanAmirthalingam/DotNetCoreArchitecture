...
Architecture with new technologies and best practices.

It is not the definitive solution for all cenarios.

It is an example to learn and apply in new projects.
...
public class UsersController : ControllerBase
{
    ...
    [AllowAnonymous]
    [HttpPost("[action]")]
    public DataResult SignIn(SignInModel signInModel)
    {
        return new DataResult(UserService.SignInJwtAsync(signInModel).Result);
    }
...
public sealed class UserService : IUserService
{
    ...
    {
        var signedInDataResult = await SignInAsync(signInModel).ConfigureAwait(false);

        if (!signedInDataResult.Success)
        {
            return new ErrorDataResult<TokenModel>(signedInDataResult.Message);
        }

        var claims = new List<Claim>();

        claims.AddSub(signedInDataResult.Data.UserId.ToString());

        claims.AddRoles(signedInDataResult.Data.Roles.ToString().Split(", "));
